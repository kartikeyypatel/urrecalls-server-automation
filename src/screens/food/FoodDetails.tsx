...
import { getAllKeys, storeProduct, getAllProducts, removeProduct, fetchRecallStatus, toggleSaveStatus, get_product_id } from '~/store'
...
export default function FoodDetails({route}: FoodDetailsScreenProps) {
  ...
  const [isRecalled, setIsRecalled] = useState(recallData.length != 0);
  const [modal_visible, set_modal_visible] = useState(false);
  const [modal_text, set_modal_text] = useState("");
  const [isSaved, setIsSaved] = useState(false);

  const [ingredient_list_visible, set_ingredient_list_visible] = useState(false);
  ...
  useEffect(()=>{
    ...
    .then((data)=>{
      ...
      let this_id: string = get_product_id(item, "food");
  ...
  const url = foodToUrl(item.primary_image.image_path)
  return (
    <PageView contentContainerStyle={{justifyContent: "flex-start"} as any}>
      ...
      <View style={{ marginTop: 1*em, flexDirection: "row", justifyContent: "space-between", alignItems: "center"}}>
        ...
        <View style={{ marginLeft: 1*em, flexDirection: "column", flexGrow: 1, flexShrink: 1}}>
          <LargeText style={{fontWeight: "bold"}}>{item.name}</LargeText>
            <NormalText>{item.brand.name}</NormalText>
        </View>
      </View>
      <Touchable 
        onPress={()=>{toggleSaved()}} 
      ...
      { 
        //  Recalled 
        recallData.length != 0 && 
        recallData.map((recall_data: any, index: number) => {
          return (
            <View key={index}>
              {/*  Initiation Date */}
              <RecallInformation 
                title={t("fooddetails_initiationdate")} 
                contentString={recall_data_parse_initiation_date(recall_data.recall_initiation_date)} 
                tooltip={t("fooddetails_initiationdate_tooltip")}/>
              {/*  Food Description */}
              <RecallInformation 
                title={t("fooddetails_description")} 
                contentString={recall_data.product_description} 
                tooltip={t("fooddetails_description_tooltip")}/>
              {/*  Recall Reason */}
              <RecallInformation 
                title={t("fooddetails_reasonrecall")} 
                contentString={recall_data.reason_for_recall} 
                tooltip={t("fooddetails_reasonrecall_tooltip")}/>
              {/*  Classification of degree of health hazard  */}
              <RecallInformation 
                title={t("fooddetails_classification")} 
                contentString={recall_data.classification} 
                tooltip={t("fooddetails_classification_tooltip")}/>
              {/*  Distribution Locations */}
              <RecallInformation 
                title={t("fooddetails_distributionlocations")} 
                contentString={recall_data.distribution_pattern} 
                tooltip={t("fooddetails_distributionlocations_tooltip")}/>
              {/*  Ingredients */}
            </View>
          )
      })}
      <RecallInformation
        title={t("fooddetails_ingredients")}
        content={ingredient_list_visible ?  //  This splits the ingredients by commas except those within square brackets
          (item.ingredient_text as string).split(/,\s*(?![^\[]*\])/g).map((ingredient, idx)=>{
          return <NormalText key={idx}>{ingredient}</NormalText>
        }): <NormalText>{t("fooddetails_ingredients_touchhint")}</NormalText>}
        onPress={()=>{
          set_ingredient_list_visible(!ingredient_list_visible);
        }}
      />
    </PageView>
  )
